...
export class Weather extends LifeforcePlugin {
  ...
  constructor(router: KoaRouter, apiKey: string) {
    super(router);
    this.apiKey = apiKey;
    this.addHandlers([
      {
  ...
  private async handleWeatherZipCode(ctx: Context, next: Next) {
    ...
    if (zip) {
      ...
      try {
        const url = `http://api.openweathermap.org/data/2.5/weather?zip=${zip}&appid=${this.apiKey}`;
  ...
  private async handleWeatherForecastZipCode(ctx: Context, next: Next) {
    ...
    if (zip) {
      ...
      try {
        const url = `http://api.openweathermap.org/data/2.5/forecast?zip=${zip}&appid=${this.apiKey}`;
        const response = await axios.get(url);
        ctx.status = 200;
        ctx.body = response.data;
      } catch (err: unknown) {
